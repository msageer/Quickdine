...
function convertQuery(sql, params) {
  ...
  newSql = newSql.replace(/login_time/g, "login_time");
...
var db = {
  ...
  prepare(sql) {
    return {
      run: async (...args) => this.run(sql, args),
      get: async (...args) => this.get(sql, args),
      all: async (...args) => this.all(sql, args)
    };
  },
...
var JWT_SECRET = process.env.JWT_SECRET || "quickdine-super-secret-key-48h";
...
var defaultFrom = process.env.SMTP_FROM || "QuickDine <noreply@quickdine.example.com>";
async function sendPlatformEmail(to, subject, html) {
  if (resend) {
    try {
      const { data, error } = await resend.emails.send({
        from: "QuickDine <onboarding@resend.dev>",
        // Users must change this to their verified domain
        to,
        subject,
        html
      });
...
var uploadDir = process.env.VERCEL ? import_path.default.join("/tmp", "uploads") : import_path.default.join(rootDir, "uploads");
...
var requireRestaurantAccess = (req, res, next) => {
  const user = req.user;
...
`);
  ...
  try {
    const restaurants = await db.all("SELECT id, name FROM restaurants WHERE slug IS NULL");
  ...
  try {
    await db.exec("ALTER TABLE subscription_plans ADD COLUMN can_use_online_payments INTEGER DEFAULT 0");
  } catch (e) {
  }
  try {
    await db.exec("ALTER TABLE restaurants ADD COLUMN tin_number TEXT");
  } catch (e) {
  }
  try {
    await db.exec("ALTER TABLE restaurants ADD COLUMN is_hotel INTEGER DEFAULT 0");
  } catch (e) {
  }
  try {
    await db.exec("ALTER TABLE menu_items ADD COLUMN cogs REAL DEFAULT 0.0");
  } catch (e) {
  }
  try {
    await db.exec("ALTER TABLE orders ADD COLUMN subtotal REAL DEFAULT 0.0");
  } catch (e) {
  }
  try {
    await db.exec("ALTER TABLE orders ADD COLUMN vat_amount REAL DEFAULT 0.0");
  } catch (e) {
  }
  try {
    await db.exec("ALTER TABLE orders ADD COLUMN state_tax_amount REAL DEFAULT 0.0");
  } catch (e) {
  }
  try {
    await db.exec("ALTER TABLE orders ADD COLUMN service_charge REAL DEFAULT 0.0");
  } catch (e) {
  }
  try {
    await db.exec("ALTER TABLE orders ADD COLUMN net_total REAL DEFAULT 0.0");
  } catch (e) {
  }
  try {
  ...
  try {
    await db.exec("ALTER TABLE orders ADD COLUMN guest_last_name TEXT");
  } catch (e) {
  }
  try {
    await db.exec("ALTER TABLE orders ADD COLUMN room_number TEXT");
  } catch (e) {
  }
  try {
    await db.exec("ALTER TABLE subscription_plans ADD COLUMN max_monthly_gmv REAL DEFAULT 0.0");
  } catch (e) {
  }
  try {
    await db.exec("ALTER TABLE subscription_plans ADD COLUMN transaction_fee_percentage REAL DEFAULT 0.0");
  } catch (e) {
  }
  try {
    await db.exec("ALTER TABLE subscription_plans ADD COLUMN is_pay_as_you_go INTEGER DEFAULT 0");
  } catch (e) {
  }
  try {
  ...
  try {
    await db.exec("ALTER TABLE restaurants ADD COLUMN subscription_plan_id INTEGER DEFAULT 1");
  } catch (e) {
  }
  try {
    await db.exec("ALTER TABLE restaurants ADD COLUMN subscription_status TEXT DEFAULT 'Active'");
  } catch (e) {
  }
  try {
    await db.exec("ALTER TABLE restaurants ADD COLUMN subscription_billing_cycle TEXT DEFAULT 'monthly'");
  } catch (e) {
  }
  try {
    await db.exec("ALTER TABLE restaurants ADD COLUMN subscription_expiry_date TEXT");
  } catch (e) {
  }
  try {
    await db.exec("ALTER TABLE restaurants ADD COLUMN vat_rate REAL DEFAULT 0");
  } catch (e) {
  }
  try {
    await db.exec("ALTER TABLE restaurants ADD COLUMN state_tax_rate REAL DEFAULT 0");
  } catch (e) {
  }
  try {
    await db.exec("ALTER TABLE restaurants ADD COLUMN is_tax_inclusive INTEGER DEFAULT 0");
  } catch (e) {
  }
  try {
    await db.exec("ALTER TABLE restaurants ADD COLUMN receipt_footer TEXT");
  } catch (e) {
  }
  try {
    await db.exec("ALTER TABLE orders ADD COLUMN gross_total REAL DEFAULT 0");
  } catch (e) {
  }
  try {
    await db.exec("ALTER TABLE orders ADD COLUMN net_total REAL DEFAULT 0");
  } catch (e) {
  }
  try {
    await db.exec("ALTER TABLE orders ADD COLUMN vat_amount REAL DEFAULT 0");
  } catch (e) {
  }
  try {
    await db.exec("ALTER TABLE orders ADD COLUMN state_tax_amount REAL DEFAULT 0");
  } catch (e) {
  }
  try {
    await db.exec("ALTER TABLE orders ADD COLUMN discount_amount REAL DEFAULT 0");
  } catch (e) {
  }
  try {
    await db.exec("ALTER TABLE orders ADD COLUMN discount_reason TEXT");
  } catch (e) {
  }
  try {
  ...
  try {
    await db.exec("ALTER TABLE users ADD COLUMN pin TEXT");
  } catch (e) {
  }
  try {
  ...
  try {
    await db.exec("ALTER TABLE users ADD COLUMN otp_code TEXT");
  } catch (e) {
  }
  try {
    await db.exec("ALTER TABLE users ADD COLUMN otp_expires_at DATETIME");
  } catch (e) {
  }
  try {
  ...
  try {
    await db.exec("ALTER TABLE users ADD COLUMN verification_expires DATETIME");
  } catch (e) {
  }
  try {
    await db.exec("ALTER TABLE platform_settings ADD COLUMN global_copyright_footer TEXT DEFAULT 'Powered by QuickDine'");
  } catch (e) {
  }
  try {
    await db.exec("ALTER TABLE platform_settings ADD COLUMN simulate_order_enabled INTEGER DEFAULT 0");
  } catch (e) {
  }
  try {
  ...
  try {
    ...
    if (planCount.count === 0) {
      ...
      await insertPlan.run("Starter", 5e3, 5e4, 2, 3e3, 30, 0, 0, 0, 0, 0);
      await insertPlan.run("Business Plus", 15e3, 15e4, 999999, 999999, 365, 1, 1, 1, 0, 0);
    } else {
  ...
  const checkExpiredSubscriptions = async () => {
    ...
    `, [now]);
      if (expiredRestaurants.length > 0) {
        const starterPlan = await db.get("SELECT id FROM subscription_plans WHERE plan_name = 'Starter' LIMIT 1");
  ...
  setInterval(checkExpiredSubscriptions, 60 * 60 * 1e3);
  ...
  try {
    let demoRes = await db.get("SELECT * FROM restaurants WHERE name = ?", ["The Great Burger"]);
    if (!demoRes) {
      const insertRestaurant = db.prepare("INSERT INTO restaurants (name, status) VALUES (?, 'Active')");
      const resId2 = (await insertRestaurant.run("The Great Burger")).lastInsertRowid;
      demoRes = { id: resId2 };
      const insertCategory = db.prepare("INSERT INTO menu_categories (restaurant_id, name) VALUES (?, ?)");
      const catId = (await insertCategory.run(resId2, "Burgers")).lastInsertRowid;
      ...
      await insertItem.run(resId2, catId, "Classic Cheeseburger", "Beef patty with cheese, lettuce, and tomato", 12.99, "Available", 15);
      await insertItem.run(resId2, catId, "Double Bacon Burger", "Two beef patties with bacon and cheese", 16.99, "Available", 20);
      const insertTable = db.prepare("INSERT INTO tables (restaurant_id, table_number, qr_token) VALUES (?, ?, ?)");
      await insertTable.run(resId2, "1", "table-1-token-xyz");
      await insertTable.run(resId2, "2", "table-2-token-abc");
    }
  ...
  const count = await db.get("SELECT COUNT(*) as count FROM restaurants");
  if (count.count === 0) {
  }
}
...
app.post("/api/auth/login", async (req, res) => {
  ...
  if (user.role === "restaurant" && !user.email_verified) {
    const restaurant = await db.get("SELECT account_verified FROM restaurants WHERE id = ?", [user.restaurant_id]);
  ...
  const token = import_jsonwebtoken.default.sign({ id: user.id, role: user.role, restaurant_id: user.restaurant_id }, JWT_SECRET, { expiresIn: "48h" });
  res.json({ success: true, user, token });
});
app.post("/api/auth/waiter-login", async (req, res) => {
  const { phone_number, pin } = req.body;
  const user = await db.get("SELECT * FROM users WHERE phone_number = ? AND pin = ? AND role = 'waiter'", [phone_number, pin]);
  if (!user) {
...
app.post("/api/auth/signup", async (req, res) => {
  ...
  try {
    ...
    const settings = await db.get("SELECT default_currency FROM platform_settings LIMIT 1");
    const defaultCurrency = settings ? settings.default_currency : "USD";
    const proPlan = await db.get("SELECT id FROM subscription_plans WHERE plan_name = 'Starter' LIMIT 1");
    ...
    const verificationExpires = /* @__PURE__ */ new Date();
    ...
    const transaction = db.transaction(async () => {
      ...
      const resId2 = (await insertRestaurant.run(restaurantName, "Pending", defaultCurrency, planId, "Active", expiryDateStr, businessType || "restaurant")).lastInsertRowid;
      ...
      await insertUser.run(email, password, "restaurant", resId2, verificationToken, verificationExpires.toISOString());
      return resId2;
    });
...
app.post("/api/auth/verify-email", async (req, res) => {
  ...
  try {
    ...
    if (new Date(user.verification_expires) < /* @__PURE__ */ new Date()) {
...
app.get("/api/admin/analytics", authenticateToken, authorizeRole(["admin", "super_admin"]), async (req, res) => {
  try {
    ...
    const totalRestaurants = await db.get("SELECT COUNT(*) as count FROM restaurants WHERE status = 'Active'");
    const totalOrders = await db.get(`SELECT COUNT(*) as count FROM orders WHERE 1=1 ${dateFilter}`, [...params]);
    const totalRevenue = await db.get(`SELECT SUM(total_amount) as total FROM orders WHERE status != 'Cancelled' ${dateFilter}`, [...params]);
    ...
    `, [...trendParams]);
    const aov = await db.get(`SELECT AVG(total_amount) as average FROM orders WHERE status != 'Cancelled' ${dateFilter}`, [...params]);
    ...
    const recentSignups = await db.all(`
      ...
      ORDER BY date(created_at) ASC
    `, [...trendParams]);
    const platformGmv = await db.get(`SELECT SUM(total_amount) as gmv FROM orders WHERE status != 'Cancelled' ${dateFilter}`, [...params]);
...
app.get("/api/restaurants", async (req, res) => {
  try {
    ...
    `);
    const platformSettings = await db.get("SELECT payment_paystack_enabled, paystack_public_key, payment_monnify_enabled, monnify_api_key, monnify_contract_code, payment_flutterwave_enabled, flutterwave_public_key FROM platform_settings LIMIT 1");
    if (platformSettings) {
  ...
  try {
    const transaction = db.transaction(async () => {
      ...
      for (const item of items) {
        if (!item.category_name || !item.name || item.price === void 0) continue;
        let cat_id = categoriesMap.get(item.category_name);
        if (!cat_id) {
          const cat = catQuery.get(restaurant_id, item.category_name);
          if (cat) {
            cat_id = cat.id;
          } else {
...
app.post("/api/restaurants/:id/waiters", authenticateToken, requireRestaurantAccess, async (req, res) => {
  ...
  try {
    ...
    const finalEmail = email || `waiter_${Date.now()}_${Math.floor(Math.random() * 1e3)}@quickdine.app`;
...
app.patch("/api/restaurants/:id/settings", authenticateToken, requireRestaurantAccess, upload.single("logo"), async (req, res) => {
  ...
  try {
    ...
    const updates = [];
    const values = [];
    if (waiter_allocation_enabled !== void 0) {
    ...
    if (payment_cash_enabled !== void 0) {
    ...
    const currentRestaurant = await db.get("SELECT account_number, bank_name, account_name FROM restaurants WHERE id = ?", [restaurant_id]);
...
app.post("/api/admin/restaurants", authenticateToken, authorizeRole(["admin", "super_admin"]), async (req, res) => {
  ...
  try {
    const defaultCurrency = await db.get("SELECT default_currency FROM platform_settings LIMIT 1");
    const plan = await db.get("SELECT id FROM subscription_plans WHERE plan_name = 'Starter' LIMIT 1");
    const expiryDate = /* @__PURE__ */ new Date();
    expiryDate.setMonth(expiryDate.getMonth() + 1);
    const expiryDateStr = expiryDate.toISOString();
    const transaction = db.transaction(async () => {
      ...
      const resId = (await db.run("INSERT INTO restaurants (name, status, currency, subscription_plan_id, subscription_status, subscription_expiry_date, business_type) VALUES (?, ?, ?, ?, ?, ?, ?)", [
        name,
        "Active",
        defaultCurrency ? defaultCurrency.default_currency : "USD",
        plan ? plan.id : 1,
        "Active",
        expiryDateStr,
...
app.delete("/api/admin/restaurants/:id", authenticateToken, authorizeRole(["super_admin"]), async (req, res) => {
  ...
  try {
    const transaction = db.transaction(async () => {
      ...
      const result2 = await db.run("DELETE FROM restaurants WHERE id = ?", [restaurant_id]);
      return result2;
    });
...
app.patch("/api/orders/:id/waiter", authenticateToken, async (req, res) => {
  ...
  try {
    const order = await db.get("SELECT restaurant_id FROM orders WHERE id = ?", [order_id]);
    if (!order) return res.status(404).json({ error: "Order not found" });
    if (req.user?.role !== "admin" && req.user?.restaurant_id !== order.restaurant_id) {
...
app.delete("/api/restaurants/:id/categories/:categoryId", authenticateToken, requireRestaurantAccess, async (req, res) => {
  const { categoryId } = req.params;
  try {
    const count = await db.get("SELECT COUNT(*) as count FROM menu_items WHERE category_id = ?", [categoryId]);
    if (count.count > 0) {
...
app.put("/api/admin/users/:id", authenticateToken, authorizeRole(["super_admin"]), async (req, res) => {
  try {
    ...
    const params = [role, name || null, restaurant_id || null];
    if (password) {
      query += ", password = ?";
...
app.patch("/api/admin/users/:id/verify", authenticateToken, authorizeRole(["super_admin"]), async (req, res) => {
  try {
    ...
    const transaction = db.transaction(async () => {
      ...
      const user = await db.get("SELECT role, restaurant_id FROM users WHERE id = ?", [id]);
...
app.post("/api/orders", async (req, res) => {
  ...
  const transaction = db.transaction(async () => {
    ...
    if (!final_table_id) {
      const existingTable = await db.get("SELECT id FROM tables WHERE restaurant_id = ? AND table_number = ?", [restaurant_id, "Takeaway/Online"]);
      if (existingTable) {
    ...
    const restaurant = await db.get("SELECT vat_rate FROM restaurants WHERE id = ?", [restaurant_id]);
    const vat_rate = restaurant?.vat_rate || 0;
    const state_tax_rate = 0;
  ...
  try {
    ...
    if (newOrder.customer_email) {
      const parsedItems = newOrderItems;
...
app.put("/api/waiter-calls/:callId/resolve", authenticateToken, async (req, res) => {
  try {
    ...
    if (req.user?.role !== "admin" && req.user?.restaurant_id !== call.restaurant_id) {
      return res.status(403).json({ error: "Unauthorized" });
    }
...
app.get("/api/restaurants/:id/analytics", authenticateToken, requireRestaurantAccess, async (req, res) => {
  ...
  try {
    ...
    const totalOrders = await db.get(`SELECT COUNT(*) as count FROM orders WHERE restaurant_id = ? ${dateFilter}`, [...params]);
    const totalRevenue = await db.get(`SELECT SUM(total_amount) as total FROM orders WHERE restaurant_id = ? AND status != 'Cancelled' ${dateFilter}`, [...params]);
    let trendDateFilter = `created_at >= date('now', '-7 days')`;
    let trendParams = [id];
    if (startDate && endDate) {
      trendDateFilter = `created_at >= ? AND created_at <= ?`;
    ...
    const topItems = await db.all(`
      ...
      LIMIT 5
    `, [...params]);
    const aov = await db.get(`SELECT AVG(total_amount) as average FROM orders WHERE restaurant_id = ? AND status != 'Cancelled' ${dateFilter}`, [...params]);
    res.json({
      totalOrders: totalOrders.count,
      totalRevenue: totalRevenue.total || 0,
      recentRevenue,
